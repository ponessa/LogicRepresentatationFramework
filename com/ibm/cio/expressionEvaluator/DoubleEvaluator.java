...
public class DoubleEvaluator extends AbstractEvaluator<Double> {
	...
	public static final Operator EXPONENT = new Operator("^", 2, Operator.Associativity.LEFT, 6);
	/** The factorial operator.*/
	public static final Operator FACTORIAL = new Operator("!", 1, Operator.Associativity.RIGHT, 6);


	/** The standard whole set of predefined operators */
	private static final Operator[] OPERATORS = new Operator[]{NEGATE, EQUALS, MINUS, PLUS, MULTIPLY, DIVIDE, EXPONENT, MODULO, FACTORIAL};
	 ...
	 * <br>The returned parameters contains all the predefined operators, functions and constants.
	 * <br>Each call to this method create a new instance of Parameters. 
	 * @param style The syle of the parameter
	 * @return a Paramaters instance
	 */
	...
	protected Double evaluate(Operator operator, Iterator<Double> operands, Object evaluationContext) {
		if (NEGATE.equals(operator) || NEGATE_HIGH.equals(operator)) {
			...
			return (operands.next().equals(operands.next())?1.0:0.0);
			//return operands.next()==operands.next()?1.0:0.0;	
		} else {
	...
	private double factorial(double n) {
		double fact = 1.0;
		if (n<0) throw new IllegalArgumentException("Factorial must be non-negative (n="+n+").");
	...
	public static void main(String[] args) {
		...
		expressionList.add("(2^3-1)*sin(pi/4)/ln(pi^2)");
		expressionList.add("(2+3)*5^2*sin(pi/4)");
		expressionList.add("2 + 3 * 5");
		expressionList.add("( 2 + 3 ) * 5");
		expressionList.add("5!");
		expressionList.add("2+5!");
		expressionList.add("(2+5)!");
		expressionList.add("5=5");
		//expressionList.add("");
		
		for (int i=0; i<expressionList.size(); i++) {
			System.out.println (expressionList.get(i)+" = "+evaluator.evaluate(expressionList.get(i)));
		}
		
		final StaticVariableSet<Double> variables = new StaticVariableSet<Double>();
		variables.set("x", 4.0);
		variables.set("y", 7.0);
		System.out.println("(y^3-1)*sin(pi/x)/ln(pi^2) = " + evaluator.evaluate("(y^3-1)*sin(pi/x)/ln(pi^2)", variables));
		...
		DoubleEvaluator simpleEvaluator = new DoubleEvaluator(params);
		
		expressionList.clear();
		expressionList.add("(3*-4)+2");
		expressionList.add("3^2");
		expressionList.add("ln(5)");
		for (int i=0; i<expressionList.size(); i++) {
			try {
				System.out.println("evaluator.evaluate("+expressionList.get(i)+")       = "+evaluator.evaluate(expressionList.get(i)));
				System.out.println("simpleEvaluator.evaluate("+expressionList.get(i)+") = "+simpleEvaluator.evaluate(expressionList.get(i)));
			} catch (IllegalArgumentException e) {
			    System.out.println (expressionList.get(i)+" is an invalid expression for simpleEvaluator");
		    }
		}
